# =============================================================================
# E07 - STR COLOR: byte order mismatch, 0xRRGGBB vs 0xBBGGRR
# =============================================================================
#
# CONCEPT:
#   mlx_string_put reads color as raw bytes in reverse order.
#   Passing 0xRRGGBB directly gives 0xBBGGRR on screen - bytes swapped.
#   Always wrap with str_color() before passing to mlx_string_put.
#
# TRY THIS:
#   1. Run the script - text renders red, but C_BLUE was passed.
#   2. Press SPACE - text renders blue, C_BLUE wrapped in str_color().
#   3. Press SPACE again to compare both side by side.
#
# CONTROLS:
...
def render_wrong() -> None:
    """Broken state: C_BLUE passed raw, renders red due to byte swap."""
    write_rect(0, 0, W, H, C_BG)
    render()
    put_text_centered("WRONG - should be blue, renders red", C_BLUE)

def render_correct() -> None:
    """Correct state: C_BLUE wrapped in str_color(), renders blue."""
    write_rect(0, 0, W, H, C_BG)
    render()
    put_text_centered("CORRECT - renders blue", str_color(C_BLUE))
